...
def train(args):
    ...
    if args.cuda:
        transformer.cpu()
    save_model_filename = "000.pth"
    save_model = os.path.join(args.save_model_path, save_model_filename)
    torch.save(transformer.state_dict(), save_model)

    print("\nDone, trained model saved at", save_model)
...
def main():
    ...
    train_arg_parser.add_argument("--content-weight", type=float, default=5e0,
                                  help="weight for content-loss, default is 7.5e0")
    train_arg_parser.add_argument("--style-weight", type=float, default=1e2,
                                  help="weight for style-loss, default is 5e2")
    train_arg_parser.add_argument("--lr", type=float, default=1e1,
